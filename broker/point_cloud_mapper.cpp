...
data::Result<render::RePointCloudObject> PointCloudMapper::map(
    ...
    for (const auto& pd : app.points) {
        render::re_scene::RePointObject inst;
        ...
        auto sf = pd.fill();
        switch (sf) {
            case scene::PointFill::Solid: inst.fill = render::re_scene::PointFill::Solid; break;
            case scene::PointFill::Hollow: inst.fill = render::re_scene::PointFill::Hollow; break;
            case scene::PointFill::GridDashed: inst.fill = render::re_scene::PointFill::GridDashed; break;
            default: inst.fill = render::re_scene::PointFill::Solid; break;
        }
        inst.fillParam = 0.0f;
