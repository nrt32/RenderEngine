...
// broker/camera_mapper.hpp — CameraMapper: ICachedMapper<scene::Camera, render::Camera> (T3 V3.2b).
//
// One file per mapper (guardrail broker_per_type). Cached (per-field viewGen/projGen)
// so a Camera::rotate(1°) dirties only viewGen, not projGen (SPEC §10.4). Pure
// translation is viewMatrix/projMatrix/position forwarding — no GL, no core/ GL
// calls (gpu_api_ownership: broker stays outside core/).
...
/// when both match last cached values.
