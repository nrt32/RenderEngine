...
- **Dependency lock** — every third-party dep pinned via FetchContent `GIT_TAG`
  (GLFW 3.4, glad2 v2.0.8, GLM 1.0.1, ImGui v1.92.9, GoogleTest v1.15.2,
   ...
   commit SHA, never a branch name** (`master|main|HEAD|develop|feature|release|hotfix|branch|stable|next|latest|trunk|dev|vNext` — V8 T8.5 hardened). No unpinned
   fetches, no vendored binaries. *Audit: `deps_pinned` (+ `deps_pinned_no_branch` — multiline-aware after V8 T8.5, system `find_package` version pin).* 
