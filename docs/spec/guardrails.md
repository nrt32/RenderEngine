...
- **Dependency lock** — every third-party dep pinned via FetchContent `GIT_TAG`
  (GLFW 3.4, glad2 v2.0.8, GLM 1.0.1, ImGui v1.92.9, GoogleTest v1.15.2,
   ...
   commit SHA, never a branch name** (`master|main|HEAD|develop|feature|release|hotfix|branch|stable|next|latest|trunk|dev|vNext|1\.x|refs/heads|origin` — V8 T8.5 hardened + spec-review #6 full denylist). No unpinned
   fetches, no vendored binaries. *Audit: `deps_pinned` (+ `deps_pinned_no_branch` multiline-aware after V8 T8.5 + `deps_pinned_no_find_package` — system `find_package` version pin, spec-review #1/6). No `find_package` fallback for `glfw3`/`glm`/`spdlog`/`nlohmann_json` (spec-review #1).* 
- **GL ownership & RHI (EOL, web-verified)** — raw `glXxx(...)` calls only under `core/rhi/gl/` (RAII GL objects + thin `core::Draw` façade over `IRHIContext` with `IRHIContext{createTexture2D/3D,createFramebuffer,createBuffer,createShader,blit,memoryBarrier,capabilities}` + `IRHIShaderDesc{stages,SPIRV|GLSL,defines}` — Qt QRhi/O3DE/Adept RHI pattern); `render/`, `app/`, `tests/`, `broker/`, `scene/` use `core/` `IRHI*` wrappers (broker forwards through `render/` helpers, never calls `gl*` directly); never in `io/`, `data/`, `volume/`, `scene/`. The stable `core/rhi/{irhi_context.hpp,irhi_framebuffer.hpp,irhi_texture.hpp,irhi_buffer.hpp,irhi_shader.hpp}` abstraction is the DIP-owned policy (Oleksii Tym "policy owns interface") — `core/rhi/gl/` and future `core/rhi/vulkan/` are interchangeable implementations (one audit anchor, OCP for graphics API swap; new RHI = new `IRHIContext` impl, zero edits to `render`/`broker`/`app` per Adept command-list RHI). *Audit: `gpu_api_ownership` (`forbid_outside core|` transitional until **(stretch) T17 RHI** lands (`core/rhi/gl/` appears), then tightens to `core/rhi/gl|` — see `audit.rules:49-50` + `guardrails.md` §6 and `TASKS.md:T17:G` `grep -c "rhi_ownership"==1 active) + `rhi_ownership` (future hard anchor `# rhi_ownership` — **activate when `core/rhi/gl/` appears at (stretch) T17; `TASKS.md:T17:G` defines uncommenting `rhi_ownership` + enforcing `require_only core/rhi/gl|` for IRHI includes; until then `core|` anchor stays**) + `require_only core/rhi/gl|` for IRHI includes (stretch deferred).* Raw `gl*` in `core/` outside `core/rhi/gl/` also fails after migration; `core::loadCoreGl`/`core::readRgba8` anchors remain as `core/rhi/gl/` facades.
- **Disposition / layer isolation (DIP)** — `scene/` is RE-free and GL-free
   (may include `data/`+`volume/`+`glm`+`data/result` only — **never `io/` or `render/` or `core/` GL**; filesystem loaders belong to `utils/` per V7 T7.1); `render/` never includes
   `scene/`; `broker/` is the **only** library that may include **both**
   `scene/` and `render/` (`app/` includes `scene/`+`broker/` but never
   `render/` directly, and depends on `IViewBridge` abstraction — DIP — never concrete `ViewBridge`). No `render/types.hpp` include inside `scene/` headers. `app → IViewBridge`, `broker → IMapper/TranslateContext` (policy owns abstraction — DIP layers). *Audit: `disposition_scene` / `disposition_render` (`forbid_inside` mechanical — `scene|#include.*render/` and `render|#include.*scene/` + `scene`→`io` via `grep -c "obj_mesh_loader" scene/ ==0` in V7 T7.1); `scene` → `core/` GL leak additionally forbidden by `gpu_api_ownership` (`forbid_outside core|` raw `gl*`) + review gate (transitional — until V3.2a `core/rhi/gl/` lands, `scene/` may not include `core/` GL headers at all).* 
...
- **Evidence rule + regression lock** (always keep, generic) — every gate
  produces verifiable evidence (analytic `1/255`/`1e-6`/`152 MB`/`BudgetExceeded` per task, `evidence_analytic` global anchor + per-task DoD `grep -c` counts are the mechanical floor; placeholders `__never_matches_*` intentionally review-gated per `tools/audit.rules:158-162` spec-review #6, stretch T18); a failing gate blocks the task; established
  behaviors must not regress without explicit approval (`regression_lock` review-gated).
...
- **Build hygiene** — warnings-as-errors, no warning-suppression flags/pragmas
   (generic built-in checks). No per-target `-fsanitize` — only `INTERFACE re_project_sanitizers` (covers `add_compile_options`/`add_definitions` **and** `target_compile_options`/`target_link_options`/`add_link_options` after V8 T8.5). *Audit: `no_per_target_sanitize`.*
- **Drift guards — sample bloat + object duplicate (T14, SPEC §6, no_sample_bloat / no_object_duplicate)** — the 11 byte-identical `scene/objects/*.hpp` headers collapsed in T5 to 6 technique kinds (`Mesh, MeshSlice, Volume, VolumeSlice, Plane, Contour`) via `MeshObject{GeometryKind}` and the 160-line `app/mesh_sample.cpp` trimmed in T7 via `Engine` facade + `SceneViewBuilder` must not re-bloat as new techniques are added; **V7 adds `Csg,Point,Line` → 9 technique kinds (`Mesh,MeshSlice,Volume,VolumeSlice,Plane,Contour,Csg,Point,Line`)** with `SceneKind::Count=9` (`Layer::Count` stays `8` — layers are stacking, kinds are dispatch; `scene/iscene_object.hpp:81` `Count=6`→`9` at `V7 T1`). The audit enforces the secondary caps via `wc` in `tools/audit.sh` (`mesh_sample.cpp ≤80`, `mpr_slice.hpp ≤100` — the waist gauge when `mpr_sample.cpp` alone cannot reach 80) while the T14 gate asserts the exact committed sizes `mesh_sample.cpp ==42` and `mpr_slice.hpp ==98` (not `≤` caps) plus `grep -c "class.*Object.*ObjectBase" scene/objects/*.hpp ==9` post-V7 (exact `9`, not `≤6` — V7 T2 adds `CsgObject`/`PointObject`/`PointCloudObject`/`LineObject`) — the precise 1/255 ordering and mask are already verified in the T8 layer gate. The duplicate guard also forbids `>10%` identical lines between any two `scene/objects/*.hpp` headers via `diff` (identical files fail). *Audit: `no_sample_bloat` + `no_object_duplicate` (`forbid_grep` placeholders, real enforcement via `wc`/`diff` in `audit.sh` plus exact `wc -l` gate; V7 updates `audit.sh` `cnt==9` check at `T2`).*
- **Technique dispatch vs layer stacking (V7, SPEC §3.1/§6)** — `SceneKind::Count=9` (`Mesh, MeshSlice, Volume, VolumeSlice, Plane, Contour, Csg, Point, Line` per `scene/iscene_object.hpp:74` — `Csg=6, Point=7, Line=8, Count=9`) while `Layer::Count` stays `8` (`LAYER_0..7` dumb `scene/layer.hpp:21` — layers are stacking, kinds are dispatch). Global `techniqueOrder` is `[Volume, VolumeSlice, Plane, Csg, Mesh, MeshSlice, Point, Line, Contour]` size `9` (`broker/render_stack.hpp:53` `array<SceneKind,9>` — `Csg` index 3 resolves before `Mesh` index 4 via `CsgOitStage` Puxel 2-stage SSBO `csgResolved` then `LinkedListOIT::endWithCsg` k-way `over()`). The `SceneKind::Count=9` vs `Layer::Count=8` divergence is intentional and mechanical: `V7 T1` gate asserts `grep -c "SceneKind::Csg\|SceneKind::Point\|SceneKind::Line" scene/iscene_object.hpp ==3` + `grep -c "SceneKind::Count.*9" scene/iscene_object.hpp ==1` + `grep -c "techniqueOrder" broker/render_stack.hpp ==1`. *Audit: `broker_per_type` one `class.*Mapper` per file, `ViewBridge` coordinator exempt, `CsgOitStage` coordinator not mapper; `gpu_api_ownership` `core|` anchor unchanged, `rhi_ownership` stays commented until `T17`.*
- **Depth default divergence (V5 T8b/T17 G4, SPEC §3.1)** — low-level `scene::View`/`render::View` default to `DepthConfig{false}` / `DepthMode::ColorOnly` (color-only deterministic, painter's order) so every analytic gate baked on `GALLIUM_DRIVER=llvmpipe` `MESA_GL_VERSION_OVERRIDE=4.6` stays byte-identical without depth interference; `scene::View` owns `DepthConfig{bool enabled{false}; float clearDepth{1.0f};}` value object (`scene/depth_config.hpp`, composition SRP, OCP for future `func/writeMask/clearDepth/stencil`) with `setDepthConfig(DepthConfig)` bumping `depthTestGen`/`generation` and syncing `depthTest` bool; `ViewTarget` `DepthMode::Enabled` owns `GL_DEPTH_COMPONENT24` at `GL_DEPTH_ATTACHMENT` (completeness-checked) and `REContext::beginPass(depthConfig)` enables+clears depth once per `View::render` before `drawLayer` loop (OIT capture/composite explicitly `disableDepthTest`). `viz::Engine` facade diverges for viz correctness — `Engine::setView(ViewDescriptor)` / `Engine::createView` default to `DepthConfig{true}` for mesh-containing views via single-site `applyMeshDepthDefault` (`v.setDepthConfig(DepthConfig{true})` in `include/render_engine/engine.hpp` — audit `engine_depth_default` enforces). Visualization consumers expect true occlusion (nearer wins) when meshes overlap; low-level stays `false` for deterministic gates. *Audit: `engine_depth_default` (`require_grep DepthConfig\{true` in `include/render_engine/engine.hpp`) + `grep -c "setDepthConfig" ==1 && grep -c "DepthConfig{true" ==1` (analytic `==1`, not `>=1`) + `grep -c "DepthConfig" scene/depth_config.hpp ==1` (file-specific, spec-review #10) `&& test $(grep -R "DepthConfig" scene/ | wc -l) -ge 1` (brittle `scene/ ==1` tightened to `>=1`).*
...
<!-- T17 stretch waived: core/rhi/gl/ RHI anchor remains future, core| stays until stretch RHI lands -->
