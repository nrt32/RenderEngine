...
Pure-redesign V3 (no new FRs — R1–R8 redesign scope per 2026-08-23 direction). The `V3.x` alias survives only for Spec traceability; the **accepted standard is `Tn: Title / D / T / G`**, global sequential `T9..T18` after `T1–T16` (V1) + `V2-T1..V2-T8` (V2). `TASKS.md` holds the binding headings — this table mirrors them. The review follow-up block keeps its original P order (17–23) while the Task ids follow the dependency-order rename in `TASKS.md` (pre-rename: 17 OIT-sample→`T19`, 18 broker→`T20`, 19 persistence→`T21`, 20 consolidation→`T17`, 21 depth→`T18`).
