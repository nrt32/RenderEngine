...
## 9.1 V3 roadmap — pure-redesign `scene`/`broker`/View/List/Persistence (renamed `V3.x` → `T1..T16` + splits =21 active +2 stretch =23 total, spec-review #11 fixed count + iteration 2 #1 `T11→T11a/b` split)

Pure-redesign V3 (no new FRs — R1–R8 redesign scope per 2026-08-23 direction). The `V3.x` alias survives only for Spec traceability (archived in `COMPLETED_TASKS.md`; active backlog is `TASKS.md` `T1..T16` inc. splits `T3a/b,T8a/b,T11a/b,T14a/b,T15a/b` = 21 active + `T17`/`T18` stretch = 23 total, spec-review gate 2026-08-29 fixed count + iteration 2 #1 `T11→T11a/b`); the **accepted standard is `Tn: Title / D / T / G`**, global sequential `T1..T16` with splits for the pure-redesign iteration after `T1–T16` (V1) + `V2-T1..V2-T8` (V2). `TASKS.md` holds the binding headings — this table mirrors them (historical alias table retained here for traceability only). The review follow-up block keeps its original P order (17–23) while the Task ids follow the dependency-order rename in `TASKS.md` (pre-rename: 17 OIT-sample→`T19`, 18 broker→`T20`, 19 persistence→`T21`, 20 consolidation→`T17`, 21 depth→`T18`). `V3.x` alias retained only here for traceability; active backlog is `T1..T16` with splits =21 active (+ stretch `T17`/`T18` =23).
...
| 2 | T2 | V3.2a | `CompositeKey` + `TranslateContext` + `REContext` skeletons (formerly `DrawContext`) — must land before any cached mapper | Land skeletons `CompositeKey{Version,LayoutId,Id,Gen,Hash}`, `TranslateContext{ViewContext{viewPlane,viewMatrix,projMatrix}, optional<VolumeContext{volumeModel,dims,voxelSpacing,meshBounds}>}` ISP-segregated (Q40:B — not God `ReView*`, not flat `viewPlane+view+volumeModel` fat), `REContext{Viewport,ClearColor,Depth,Blend,spy}` instance per `FrameContext` (formerly `DrawContext`) replacing `core/draw.cpp` static `invalidateDrawCache()` global (SRP via instance — Q43:B). Value types, header-only, unblock `T3`/`T5`/`T6`. | T1 | §10.1, §10.4, §11.4 |
...
| 9 | T9 | V3.8 | RE-minimal types — `render/re_scene/` inventory | Audit `scene→render`: `Re*` keeps only `Re`-direct values (`AssetHandle`/`ReMaterial*`/`ClipPlane`/`worldBounds`/`sliceUVW` where derived), never `scene::MaterialDesc`. Produce binding inventory `docs/re_scene_inventory.md` (6 tables/23 fields). | T1, T5, T7, T8 | §12.4 |
| 10 | T10 | V3.9 | EOL skeletons — (stretch) deferred | `RHI`/`IJobExecutor`/serialisation skeletons **deferred** (stretch, T17 — was `T10` per `roadmap.md:46`, iteration 6 #7) — only `REContext` (formerly `DrawContext`)/`IDirtyTracker`/`Version` extension points remain. | T2, T6 | §3, §11.6, §13.8 |
...
| 17 | T19 | Review | OIT sample — opaque + transparent meshes with depth overlap | ≥2 opaque + ≥2 transparent real meshes interleaved along the view direction (replaces three transparent quads); analytic composite probes; consumes the depth support task. | T18 | FR-render.2/3 |
| 18 | T20 | Review | Broker = only app path; complete mapper inventory | Implement missing `PlaneMapper` (+ volume layer reality), delete Noop renderables from `ViewSynchronizer`, route all samples through `IViewBridge` (today broker has zero app consumers). | T3, T16 | §3, §11 |
| 19 | T21 | Review | Persistence honesty — activate scaffolding | `dirtyFieldsSince` computed from `dirtyLog_`; tombstones enforced in `resolve`; single shared `StableKey` (divergent twins unified); fake `parallelFor` + discarded dirty results removed; id-keyed camera cache. | T6 | §10.7 |
| 20 | T17 | Review | Renderer consolidation | One pass-prologue, one shared quad, one `geometryFor`, one content-hash definition (`data/content_hash.hpp`), merged `render()`/`drawLayer` bodies, `<glad/gl.h>` out of `render/`. Zero pixel drift. | T5 | §3 |
| 21 | T18 | Review | Depth-buffer support — optional attachment + per-view flag | Color-only stays default (analytic gates); opt-in depth target for true occlusion (near-mesh-wins gate); feeds OIT-with-opaque-meshes and unblocks removing the MPR box face-ordering hack. (Renumbered from the pre-dependency-order id T21 — `TASKS.md` holds the binding headings.) | T5 | §3 |
