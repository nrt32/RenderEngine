...
**Remaining grill (must pin before V3 kicks off — see §13):** None for §10 — all §10 Qs resolved. Remaining EOL grill is §13 Q31/Q32/Q33/35/36/38 (now with binding recommendations; product must confirm typed `unsupported` vs silent degrade for Q32, JSON+binary codec for Q36, and `ITypeErasedDraw` vs `std::function` for Q38 before header review).
