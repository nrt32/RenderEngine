...
// scene/objects/cube_object.hpp — CubeObject concrete kind (T1).
//
...
// The registrar plus SceneFactory entry make Factory::create(Cube) succeed
...
/// CubeObject — see header comment for role and open-extension guarantee.
class CubeObject : public ObjectBase<CubeObject> {
   public:
    static constexpr SceneKind Kind = SceneKind::Cube;

    CubeObject() = default;
    CubeObject(const CubeObject&) = default;
    CubeObject(CubeObject&&) noexcept = default;
    CubeObject& operator=(const CubeObject&) = default;
    CubeObject& operator=(CubeObject&&) noexcept = default;
...
REGISTER_SCENE_OBJECT(CubeObject)
