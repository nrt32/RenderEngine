...
// uncompressed voxel block, SPEC S7) into a data::VolumeDataset (CPU
// container).
//
...
//   - "encoding: raw" only (SPEC S7: raw, uncompressed voxel block; gzip/bzip2/
...
// memory budget cap (SPEC S5: every axis <= 128 and the product <= 128^3) is
...
/// Enumerated diagnostics codes carried by data::Error::code for NRRD load
...
enum class VolumeLoadError : int {
    ...
    UnsupportedEncoding = 6,  ///< encoding is not "raw" (v1, SPEC S7).
